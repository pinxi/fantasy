...
const leaguesJob: JobSpec = {
  ...
  async run(ctx): Promise<JobReport> {
    ...
    for (const league of list) {
      ...
      const users = (await getArchived(ctx, 'league_users', league.league_id, `${BASE}/league/${league.league_id}/users`)) as Array<
      ...
      const rosterList = (await getArchived(ctx, 'rosters', league.league_id, `${BASE}/league/${league.league_id}/rosters`)) as Array<
        Record<string, unknown>
      >;
      ...
      const draftList = (await getArchived(ctx, 'drafts', league.league_id, `${BASE}/league/${league.league_id}/drafts`)) as Array<
        Record<string, unknown>
      >;
    ...
    for (const league of current) {
      for (let week = 1; week <= maxWeek; week++) {
        const data = (await getArchived(
          ctx,
          'transactions',
          `${league.leagueId}-w${week}`,
          `${BASE}/league/${league.leagueId}/transactions/${week}`,
        ...
        for (const t of data ?? []) {
          ctx.db
            ...
            .onConflictDoUpdate({
              target: [transactions.leagueId, transactions.transactionId],
              set: { status: (t.status as string) ?? null, fetchedAt: Date.now() },
            })
            .run();
          written++;
        }
...
export const sleeperJobs: JobSpec[] = [playersDump, leaguesJob, matchupsJob, backfill2025, transactionsJob, trendingJob];
